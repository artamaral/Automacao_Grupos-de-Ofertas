...
from ofertas_bot.providers.provider_settings import get_provider_graphql_urls  # noqa: E402
...
from ofertas_bot.providers.transport import HttpTransportError, UrllibHttpTransport  # noqa: E402
from ofertas_bot.settings import Settings  # noqa: E402
    ...
    *,
    ...
    request = build_graphql_request(
        ...
        signer=ShopeeGraphqlSigner(
            credential=provider.settings.shopee_partner_id or "",
            api_secret=provider.settings.shopee_secret_key or "",
        ),
        timestamp=int(time()),
        query=query,
    ...
    return UrllibHttpTransport().send(request)


def _get_repo_settings() -> Settings:
    load_dotenv(ROOT_DIR / ".env", override=False)
    return Settings(_env_file=ROOT_DIR / ".env")
