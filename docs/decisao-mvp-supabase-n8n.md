...
- `cooldown_until` retira o item dos quatro proximos dias operacionais, de forma
...
flowchart TD
    ...
    P --> Q[cooldown_until: meia-noite BRT de D mais 5]
    Q --> R[Item inelegivel nos quatro proximos dias]
    R --> S[Item volta ao ranking no quinto dia]
    S --> H
```
...
96 horas. Um item publicado no dia `14` fica fora dos planos dos dias `15`,
`16`, `17` e `18` e retorna no dia `19`; seu `cooldown_until` sera
`19 00:00 BRT`.
