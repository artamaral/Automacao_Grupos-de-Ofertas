...
| `docs/status-guarda-http-real.md` | Documento de transição | Registra a etapa da guarda de HTTP real integrada aos providers. | Integrar ao status principal quando possível; remover depois da integração. |
| `docs/status-transport-http-real.md` | Documento de transição | Registra a conexão controlada do transport HTTP real atrás da guarda. | Integrar ao status principal quando possível; remover depois da integração. |
...
| `real_http_guard.py` | Guarda de segurança | Bloqueia HTTP real quando pré-requisitos mínimos não estão seguros. | Manter como trava antes de conectar qualquer transport real. |
| `UrllibHttpTransport` atrás da guarda | Transport real controlado | Prepara chamada externa somente quando a guarda e a flag permitem. | Manter desligado por padrão e validar em diagnóstico controlado. |
