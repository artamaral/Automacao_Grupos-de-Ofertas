...
begin
  ...
  if v_confirmation_count = 0 then
    ...
    set selected_at = null,
        ...
        selection_reason = case
          ...
          when state.selection_bucket = 'publication_cooldown_2d' then null
  ...
  v_cooldown_until := (
    ((v_last_sent_at at time zone 'America/Sao_Paulo')::date + 3)::timestamp
...
create or replace function offers.sync_offer_publication_state()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  if tg_op = 'DELETE' then
    if old.profile = 'feminino' and old.marketplace = 'shopee' then
  ...
  if tg_op = 'UPDATE'
    and (old.profile, old.marketplace, old.stable_key)
      is distinct from (new.profile, new.marketplace, new.stable_key)
    and old.profile = 'feminino'
    and old.marketplace = 'shopee'
  then
    perform offers.reconcile_offer_publication_state(
      old.profile,
      old.marketplace,
  ...
  if new.profile = 'feminino' and new.marketplace = 'shopee' then
    perform offers.reconcile_offer_publication_state(
      new.profile,
      new.marketplace,
      new.stable_key
  ...
  for v_stable_key in
    ...
    from (
      ...
      where state.profile = p_profile
        and state.marketplace = p_marketplace
        and (
          state.selection_bucket = 'publication_cooldown_2d'
...
drop trigger if exists publication_events_sync_offer_selection_state
  on offers.publication_events;

create trigger publication_events_sync_offer_selection_state
after insert or update or delete
on offers.publication_events
for each row execute function offers.sync_offer_publication_state();

comment on function offers.reconcile_offer_publication_state(text, text, text) is
  'Reconstroi o estado operacional de publicacao de um item a partir do ledger confirmado.';
comment on function offers.rebuild_offer_publication_state(text, text) is
  'Reconcilia em lote o cooldown de publicacao; executar fora da janela diaria em andamento.';
comment on trigger publication_events_sync_offer_selection_state
  on offers.publication_events is
  'Projeta confirmacoes do feminino/shopee em offer_selection_state sem alterar o ledger.';

do $$
begin
  revoke all on function offers.reconcile_offer_publication_state(text, text, text)
    from public;
  revoke all on function offers.sync_offer_publication_state() from public;
  revoke all on function offers.rebuild_offer_publication_state(text, text)
    from public;

  if exists (select 1 from pg_roles where rolname = 'anon') then
    revoke execute on function offers.reconcile_offer_publication_state(text, text, text)
      from anon;
    revoke execute on function offers.sync_offer_publication_state() from anon;
    revoke execute on function offers.rebuild_offer_publication_state(text, text)
      from anon;
  end if;
  ...
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    revoke execute on function offers.reconcile_offer_publication_state(text, text, text)
      from authenticated;
    revoke execute on function offers.sync_offer_publication_state() from authenticated;
    revoke execute on function offers.rebuild_offer_publication_state(text, text)
      from authenticated;
  end if;
end;
