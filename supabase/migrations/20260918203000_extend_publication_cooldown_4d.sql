...
begin
  ...
  v_cooldown_until := (
    ((v_last_sent_at at time zone 'America/Sao_Paulo')::date + 5)::timestamp
...
comment on function offers.reconcile_offer_publication_state(text, text, text) is
  'Reconstroi o estado operacional de publicacao de um item a partir do ledger confirmado com cooldown de quatro dias operacionais.';
comment on function offers.rebuild_offer_publication_state(text, text) is
  'Reconcilia em lote o cooldown de publicacao de quatro dias operacionais; executar fora da janela diaria em andamento.';
