...
select
  ...
  concat_ws(
    E'\n',
    ready.product_name,
    case when ready.price is not null then 'Hoje: R$ ' || ready.price::text end,
    case when ready.rating is not null then 'Avaliacao: ' || ready.rating::text || '/5' end,
    case
      when btrim(coalesce(ready.offer_link, '')) <> ''
        then concat_ws(E'\n', 'Link da oferta:', ready.offer_link)
    end
  ) as caption_base
