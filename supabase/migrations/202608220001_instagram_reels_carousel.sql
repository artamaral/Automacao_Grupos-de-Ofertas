...
select
  ...
  concat_ws(
    ...
    '#ad #shopee #' ||
      coalesce(
        nullif(regexp_replace(lower(coalesce(plan.primary_subniche, 'ofertas')), '[^a-z0-9]+', '', 'g'), ''),
        'ofertas'
      ) || ' #achadinhos'
  ) as caption_base,
...
cross join lateral (
  ...
  where jsonb_array_length(media.image_urls) >= 4
) format
where media.status = 'valid'
order by plan.planned_date, plan.daily_sequence, format.instagram_format desc;

comment on view offers.v_instagram_dispatch_ready is
  'Superficie pronta para Instagram: usa o plano diario e midia valida; dispatch_status e planned_hour nao definem elegibilidade.';

create unique index if not exists publication_events_instagram_source_format_idx
  on offers.publication_events (
    channel_adapter,
    ((payload ->> 'source_dispatch_plan_id'))
  )
